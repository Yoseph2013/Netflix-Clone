        ...
        <div className="aspect-video mb-8 p-2 sm:px-10 md:px-32">
          ...
          {trailers?.length === 0 && (
            <h2 className="text-xl text-center mt-5">
              ...
              <span className="font-bold text-red-500">{content?.title || content?.name}</span>
            </h2>
          )}
        </div>
          ...
          <div className="mb-4 md:mb-0">
            ...
            <p className="mt-2 text-lg">
              ...
              {content?.adult ? (
                ...
                <span className="text-gray-600">PG-13</span>
              )}{" "}
            </p>
            <p className="mt-4 text-lg">{content?.overview}</p>
            </div>
            <img
						src={ORGINAL_IMG_BASE_URL + content?.poster_path}
						alt='Poster image'
  ...
  <div className="mt-12 max-w-5xl mx-auto relative">
    ...
    <div className="flex overflow-x-scroll scrollbar-hide gap-4 pb-4 group" ref={sliderRef}>
      {similarContent.map((content) => (
        <Link key={content.id} to={`/watch/${content.id}`} className="w-52">
          <img
      ...
      text-white rounded-full " onClick={scrollRight}/>

   <ChevronLeft 
      className="absolute top-1/2 -translate-y-1/2 right-2 w-8 h-8
      opacity-0 group-hover:opacity-100 transition-all duration-300 cursor-pointer bg-red-600
      text-white rounded-full " onClick={scrollLeft}/>
    </div>
  </div>
)}
